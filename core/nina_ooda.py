...
# Helper to run async in thread if needed
...
def is_task_fixed(task_id: str) -> bool:
    commit_index = pathlib.Path("nina_commit_index.md")
...
def observe() -> dict:
    graph_path = pathlib.Path("nina_context_graph.json")
    if not graph_path.exists() or (datetime.datetime.now() - datetime.datetime.fromtimestamp(graph_path.stat().st_mtime)).total_seconds() > 7200:
        # Regenerate context graph
        subprocess.run(["python3", "nina_context_graph.py"], capture_output=True, check=False)
    ...
    for node_id, node in nodes.items():
        ...
        if node_id in cooldowns:
            try:
                ...
                if (now - last_act_time).total_seconds() < 21600: # 6 hours cooldown
                    continue
            except Exception:
                pass
                
        # Integrate Predictive AST Risk Analysis
    ...
    if action == "ALERT_BOSTAMI":
        ...
        if backlog_path.exists():
            ...
            if task_id not in content:
                ...
                # Append card before the Summary Snapshot table if present
