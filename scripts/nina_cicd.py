...
async def dispatch_ready_tasks() -> list:
    ...
    for task_id in ready_tasks:
        ...
        # Get task details from register
        # We can run a simple dispatch via Jules
...
async def poll_and_merge() -> list:
    ...
    # Poll open Jules PRs via gh CLI
    try:
        res = subprocess.run(
            ...
            for pr in prs:
                ...
                if task_id.lower() in head.lower() or task_id.lower() in title.lower():
                    ...
                    if merge_res.returncode == 0:
                        ...
                        subprocess.run(["bash", "./nina_sync.sh"], cwd=REPO_ROOT, check=False)
