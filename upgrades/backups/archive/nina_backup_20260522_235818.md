...
- `core/agent.py` — 93 lines — last modified: 2026-05-22 21:45:02
...
- `core/router.py` — 295 lines — last modified: 2026-05-22 22:59:26
...
- `healthcheck.py` — 178 lines — last modified: 2026-05-22 23:56:21
- `idleloop.py` — 133 lines — last modified: 2026-05-22 21:45:02
...
- `interfaces/telegram_interface.py` — 372 lines — last modified: 2026-05-22 21:45:02
- `main.py` — 29 lines — last modified: 2026-05-22 23:56:14
...
> Last modified: 2026-05-22 21:45:02 | Lines: 93
...
> Last modified: 2026-05-22 22:59:26 | Lines: 295
...
> Last modified: 2026-05-22 23:56:21 | Lines: 178

```python
#!/usr/bin/env python3
"""NINA v12 - Auto-Discovery Health Check"""
import ast, importlib, inspect, sys, time
from pathlib import Path
...
def ok(name, detail=''):
    PASSED.append(name)
    print('  [32mOK[0m  ' + name + (' - ' + detail if detail else ''))

def fail(name, reason):
    FAILED.append((name, reason))
    print('  [31mFAIL[0m  ' + name)
    print('       [33m' + reason + '[0m')
...
def critical(name):
    def decorator(fn):
        try:
            ...
            fail(name, type(e).__name__ + ': ' + str(e))
    return decorator

SKIP_DIRS = {'venv', '.venv', 'env', '__pycache__', 'archive', 'upgrades'}
...
print('[1] Static analysis - scanning for undefined names...')

def check_undefined_names(path):
    try:
        tree = ast.parse(path.read_text(), filename=str(path))
    except SyntaxError as e:
        fail(str(path.relative_to(ROOT)) + ' - syntax', str(e)); return
    ...
    ALWAYS = set(dir(_bi)) | {
        ...
        '__loader__', '__builtins__', 'TYPE_CHECKING', 'annotations',
        'Self', 'dataclass', 'field', 'property', 'staticmethod',
        'classmethod', 'abstractmethod', 'overload',
    }
    suspicious = {n for n in (used - defined - ALWAYS) if len(n) > 1 and not n.startswith('_')}
    rel = path.relative_to(ROOT)
    if suspicious:
        fail(str(rel) + ' - possibly undefined names', 'Check: ' + ', '.join(sorted(suspicious)[:8]))
    else:
        ok(str(rel) + ' - no undefined names')

for f in get_py_files():
    check_undefined_names(f)

print('')
print('[2] Import check - every module must load cleanly...')

MODULES = []
for pyfile in get_py_files():
    rel = pyfile.relative_to(ROOT)
    parts = list(rel.parts)
    mod = '.'.join(parts[:-1]) if parts[-1] == '__init__.py' else '.'.join(parts).replace('.py', '')
    if mod: MODULES.append((mod, str(rel)))

for mod, rel in MODULES:
    try:
        importlib.import_module(mod)
        ok(rel + ' imports cleanly')
    except Exception as e:
        fail(rel + ' import failed', type(e).__name__ + ': ' + str(e))

print('')
print('[3] Type registry - cataloguing constants...')
...
for mod, rel in MODULES:
    ...
    for name, val in inspect.getmembers(m):
        ...
        if inspect.ismodule(val) or inspect.isfunction(val) or inspect.isclass(val): continue
        TYPE_REGISTRY.append((rel, name, type(val).__name__, repr(val)[:60]))

registry_path = ROOT / 'data' / 'healthcheck_registry.txt'
registry_path.parent.mkdir(parents=True, exist_ok=True)
with open(registry_path, 'w') as rf:
    rf.write('NINA Health Check - Type Registry' + chr(10))
    rf.write('Generated: ' + time.strftime('%Y-%m-%d %H:%M:%S') + chr(10))
    rf.write('=' * 70 + chr(10))
    cur = None
    for rel, name, typ, preview in sorted(TYPE_REGISTRY):
        if rel != cur:
            rf.write(chr(10) + '-- ' + str(rel) + chr(10)); cur = rel
        rf.write('  ' + name.ljust(35) + ' ' + typ.ljust(15) + ' ' + preview + chr(10))
ok('Type registry written - ' + str(len(TYPE_REGISTRY)) + ' entries')

print('')
print('[4] Critical checks - known failure points...')

@critical('ResponseCache._s is dict instance')
def _():
    from core.router import ResponseCache
    c = ResponseCache()
    assert isinstance(c._s, dict)
    c.set('x', 'quick', 'y', 'TEST')
    assert c.get('x', 'quick') == 'y'

@critical('ClassifiedTask.task_type field exists')
def _():
    from core.router import ClassifiedTask
    assert hasattr(ClassifiedTask('general', 100, False, False), 'task_type')

@critical('tools.browser._is_internal defined')
def _():
    import tools.browser as b
    assert hasattr(b, '_is_internal')
    assert b._is_internal('http://localhost:11434')
    assert not b._is_internal('https://google.com')

@critical('tools.shell ALLOWED_* all defined as sets')
def _():
    import tools.shell as s
    for n in ('ALLOWED_BASES', 'ALLOWED_SYSTEMCTL_SUBS', 'ALLOWED_OLLAMA_SUBS'):
        assert hasattr(s, n), n + ' missing'
        assert isinstance(getattr(s, n), set), n + ' not a set'

@critical('config tokens present in .env')
def _():
    from core.config import load_config
    cfg = load_config()
    assert cfg.telegram_bot_token and cfg.authorized_user_id

total = len(PASSED) + len(FAILED)
print('')
print('=' * 50)
print('  Passed : ' + str(len(PASSED)) + '/' + str(total))
print('  Failed : ' + str(len(FAILED)) + '/' + str(total))
if FAILED:
    print('')
    print('[31mFailed checks:[0m')
    for name, reason in FAILED:
        print('  FAIL ' + name)
        print('       ' + reason)
    print('')
    print(str(len(FAILED)) + ' check(s) FAILED - fix before starting NINA.')
    sys.exit(1)
else:
    print('')
    print('[32mAll ' + str(total) + ' checks passed - NINA is healthy.[0m')
...
> Last modified: 2026-05-22 21:45:02 | Lines: 133
...
> Last modified: 2026-05-22 21:45:02 | Lines: 372
...
> Last modified: 2026-05-22 23:56:14 | Lines: 29
...
> Last modified: 2026-05-22 23:55:00 | Lines: 32

```python
"""NINA v12 - tools/safe_tool.py
Catches NameError / AttributeError / TypeError and returns readable string.
    ...
    def decorator(fn):
        ...
        async def wrapper(*args, **kwargs):
            try:
                ...
                msg = "[" + tool_name + "] NameError: " + str(e)
                logger.error(msg, exc_info=True); return msg
            except AttributeError as e:
                msg = "[" + tool_name + "] AttributeError: " + str(e)
                logger.error(msg, exc_info=True); return msg
            except TypeError as e:
                msg = "[" + tool_name + "] TypeError: " + str(e)
                logger.error(msg, exc_info=True); return msg
            except Exception as e:
                msg = "[" + tool_name + "] Error: " + type(e).__name__ + ": " + str(e)
                logger.error(msg, exc_info=True); return msg
        return wrapper
    return decorator
...
> Last modified: 2026-05-22 23:18:57 | Lines: 67
