# NINA v12 — MASTER PROMPT
### Version: 12.1.0 · Last updated: 2026-05-20 +0600
...
class NinaConfig(BaseModel):
    ...
    api_rate_limit_rpm:  int = 60            # NEW v12: FastAPI rate limit
    ...
    idle_threshold_min: int = 15            # minutes of no user message before idle mode activates
    idle_report_min:    int = 30
    idle_auto_approve:  bool = False
    session_max_turns:  int = 20            # NEW v12: configurable session history limit

    # Agent loop
    agent_timeout_s:    int = 300           # NEW v12: global agent loop wall-clock timeout
    ...
    # Thermal guard (v12.1) — all hot-reloadable
    thermal_warn_cpu:     int = 80    # log warning + notify
    thermal_warn_gpu:     int = 80
    thermal_guard_cpu:    int = 90    # ban LOCAL_HEAVY, force LOCAL_FAST + cloud
    thermal_guard_gpu:    int = 85
    thermal_critical_cpu: int = 95   # abort agent loop
...
THERMAL_WARN_CPU=80
THERMAL_WARN_GPU=80
THERMAL_GUARD_CPU=90
THERMAL_GUARD_GPU=85
THERMAL_CRITICAL_CPU=95
THERMAL_CRITICAL_GPU=90
```
...
```
● NINA v12 ONLINE — {datetime}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Local    : qwen2.5:1.5b · qwen2.5:7b
           GPU layers: fast={fast_layers} heavy={heavy_layers}
...
Thermal  : CPU {cpu_temp_c}°C {cpu_temp_status}  GPU {gpu_temp_c}°C {gpu_temp_status}
Disk     : {disk_used_pct}%
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Ready.
```
...
`fast={fast_layers} heavy={heavy_layers}` replaces the ambiguous `{fast_layers}/{heavy_layers}` fraction format.
...
- OPENROUTER: on /addkey, scans /v1/models, registers all price==0 models as sub-providers.
  Sub-providers use conservative defaults: rpm=10, min_spacing_s=6.
...
"Idle" is defined as: no user message received for `idle_threshold_min` minutes.
...
Cache is also fully cleared on /reset.
...
Cost tracker state is persisted to data/jobs.sqlite on each write to survive restarts.
...
On classification failure → fall through to general_task AND log to nina.log:
```python
logger.warning(
    f"nlp_classification_failed: input='{text[:80]}' — falling through to general_task",
    extra={"log": "nina.log"}
)
```
...
4. router.cache.clear()                    ← v12: cache cleared on reset
5. reply: "✅ Reset complete. Memory and cache cleared. Backup at: {memory_backup_path}"
```
...
session_history in-memory only. Max `session_max_turns` turns (configurable, default 20; prune from front).
...
Reports: RAM, VRAM, CPU, disk, uptime, top 5 processes, NINA process footprint, thermal state.
...
CPU temp > 80°C   → "🌡️ CPU temp warning: {n}°C"        ← v12.1
GPU temp > 80°C   → "🌡️ GPU temp warning: {n}°C"        ← v12.1
```
...
async def get_temps() -> dict:
    ...
    if hasattr(psutil, "sensors_temperatures"):
        ...
        core = t.get("coretemp") or t.get("k10temp") or t.get("cpu_thermal")
        if core:
            temps["cpu"] = max(s.current for s in core)
    # GPU via nvidia-smi (already in shell allowlist — no new security rules needed)
    try:
        ...
        pass   # sensor unavailable — caller skips that tier silently
    return temps
```

Unavailable sensors return None; callers treat None as "skip this tier, do not fault."

### get_status() thermal section (v12.1)
```python
"thermal": {
    "cpu_c":      temps["cpu"],
    "gpu_c":      temps["gpu"],
    "cpu_status": "OK" if (temps["cpu"] or 0) < config.thermal_warn_cpu else "WARN",
    "gpu_status": "OK" if (temps["gpu"] or 0) < config.thermal_warn_gpu else "WARN",
}
```
...
### Thermal preflight guard (v12.1 — runs after RAM guard, before step loop)
```python
temps = await SystemTool.get_temps()
cpu_t = temps.get("cpu") or 0
gpu_t = temps.get("gpu") or 0

if cpu_t >= config.thermal_critical_cpu or gpu_t >= config.thermal_critical_gpu:
    await notify_telegram(
        f"🌡️ CRITICAL temp CPU={cpu_t}°C GPU={gpu_t}°C. Agent loop aborted."
    )
    logger.critical(
        f"agent_loop_aborted_thermal: CPU={cpu_t} GPU={gpu_t}",
        extra={"log": "agent.log"}
    )
    return "⚠️ Agent loop aborted: hardware temperature critical."

force_local_fast = False
if cpu_t >= config.thermal_guard_cpu or gpu_t >= config.thermal_guard_gpu:
    await notify_telegram(
        f"🌡️ HIGH temp CPU={cpu_t}°C GPU={gpu_t}°C. Forcing LOCAL_FAST + cloud only."
    )
    force_local_fast = True   # LOCAL_HEAVY banned for this invocation

if cpu_t >= config.thermal_warn_cpu or gpu_t >= config.thermal_warn_gpu:
    logger.warning(
        f"thermal_warn: CPU={cpu_t}°C GPU={gpu_t}°C",
        extra={"log": "nina.log"}
    )
# If sensor unavailable (cpu_t==0 or gpu_t==0), tier is skipped — no false aborts
```

### Global timeout guard (v12)
```python
async def run_agent_loop(goal: str, task_type: str, session_history: list) -> str:
    try:
        return await asyncio.wait_for(
            _agent_loop_inner(goal, task_type, session_history),
            timeout=config.agent_timeout_s
        )
    except asyncio.TimeoutError:
        logger.warning(
            f"agent_loop_timeout: goal='{goal[:80]}' exceeded {config.agent_timeout_s}s",
            extra={"log": "agent.log"}
        )
        partial = _get_partial_result()
        notice = f"⚠️ Task timed out after {config.agent_timeout_s}s."
...
10 core jobs (v12.1: thermal_health added):
```
...
provider_hunter   every 24h at 02:00 Dhaka (scan for new free providers)
thermal_health    every 5min (lightweight — no LLM involved)        ← v12.1
```
...
EWS email fetch — failure handling (v12):
```python
async def fetch_mailbox_summary(mailbox: str) -> str:
    try:
        return await office_mail.fetch_summary(mailbox)
    except Exception as e:
        logger.warning(f"morning_report_ews_fetch_failed: {mailbox} — {e}",
                       extra={"log": "error.log"})
        return "Unavailable (EWS unreachable — retry at next report)"
```
...
## 5.6 THERMAL HEALTH JOB (v12.1)

```python
async def thermal_health_check():
    temps = await system_tool.get_temps()
    if temps["cpu"] and temps["cpu"] >= config.thermal_warn_cpu:
        logger.warning(
            f"thermal_check: CPU={temps['cpu']}°C",
            extra={"log": "nina.log"}
        )
        await notify_telegram(f"🌡️ CPU temp: {temps['cpu']}°C — approaching limit.")
    if temps["gpu"] and temps["gpu"] >= config.thermal_warn_gpu:
        logger.warning(
            f"thermal_check: GPU={temps['gpu']}°C",
            extra={"log": "nina.log"}
        )
        await notify_telegram(f"🌡️ GPU temp: {temps['gpu']}°C — approaching limit.")
```

Runs every 5 minutes. No LLM call involved. Sensor unavailability is skipped silently.
...
Runs on schedule: daily at 02:00 Dhaka (see Stage 5 scheduler).
NLP-triggered on demand: "hunt for new providers".
...
## 6.6 CONFIG HOT RELOAD (`core/config.py`)

ConfigHotReload watches `.env` for changes using a file-modification timestamp check every 60 seconds.

Reloadable fields (no restart required):
```
ews_max_emails, ews_keywords, idle_threshold_min, idle_report_min,
idle_auto_approve, flood_window_s, flood_max_messages,
...
```
telegram_bot_token, authorized_user_id, ollama_host,
all *_api_key fields, api_secret_key, ews_server, ews_domain,
ews_username, ews_password, ews_auth_type
```

On reload success: log to nina.log with tag `config_hot_reload`, list changed fields.
On validation failure: keep old config, log error, notify Telegram.

NLP-triggered: "reload config".

---

## 6.7 AB SHADOW TESTER (`tools/upgrade_pipeline.py`)

ABShadowTester runs a candidate module alongside the current production module for a fixed number of live requests before promoting.

```python
class ABShadowTester:
    candidate_module: str          # path to sandboxed candidate
    shadow_n: int = 20             # requests to shadow before decision
    results: list[ShadowResult]    # outcome per shadowed request

    async def shadow_request(self, input_str: str) -> ShadowResult:
        prod_out   = await _call_module(self.production_path, input_str)
        cand_out   = await _call_module(self.candidate_module, input_str)
        return ShadowResult(input=input_str, prod=prod_out, candidate=cand_out,
                            match=(prod_out == cand_out))

    def summary(self) -> str:
        match_rate = sum(r.match for r in self.results) / len(self.results)
        return f"Shadow test: {len(self.results)} requests · {match_rate:.0%} match rate"
```

After `shadow_n` requests: present summary to user and await /approve or /reject.
On /reject: discard candidate, restore production.
ABShadowTester is opt-in; standard /patch flow does not require it.

---

## 6.8 CAPABILITY REGISTRY (`data/capabilities.json`)

CapabilityRegistry tracks which tool modules are loaded, healthy, and what each can do.
Used by the task classifier and agent loop to route tasks to available tools only.

```json
{
  "shell":   {"loaded": true,  "healthy": true,  "last_checked": "2026-05-20T09:00:00"},
  "browser": {"loaded": true,  "healthy": true,  "last_checked": "2026-05-20T09:00:00"},
  "email":   {"loaded": true,  "healthy": false, "last_checked": "2026-05-20T09:00:00",
              "error": "EWS connection refused"},
  "gpu":     {"loaded": true,  "healthy": true,  "last_checked": "2026-05-20T09:00:00"}
}
```

Updated on startup and after every tool health probe.
If a tool is `healthy: false`, the agent loop skips it and logs a warning rather than crashing.
NLP-triggered: "show tool status".
...
```
logs/nina.log           Main runtime log (includes NLP classification failures)
...
logs/error.log          Exceptions only (market fetch failures, EWS failures, disk guard events)
logs/security.log       Unauthorized access + authorized-user flood + API rate-limit breaches
```

Rotation: TimedRotatingFileHandler, 7-day retention per log file.

### router.log JSON schema (v12)

Each entry is one JSON object per line:
```json
{
  "ts":           "2026-05-20T09:01:34.123+06:00",
  "provider":     "CEREBRAS",
  "task_type":    "research",
  "input_tokens": 412,
  "output_tokens": 187,
...
On failure: `status` = `"failure"`, `error` = short reason string, token/cost fields = 0.
All timestamps are ISO 8601 in UTC+6.
...
Rate limiting (v12): 60 req/min per source IP (configurable via api_rate_limit_rpm).
On breach: HTTP 429 with `Retry-After` header. Logged to logs/security.log.
...
It must not block the morning report because a single market data source or EWS mailbox failed.
It must not run an agent loop indefinitely when providers are hung.
It must not route through OPENAI unless the key is present and the task is non-sensitive.
It must not run LOCAL_HEAVY when CPU or GPU temperature is at guard level or above.
It must not abort the agent loop for thermal reasons when sensors are unavailable — only act on confirmed readings.
