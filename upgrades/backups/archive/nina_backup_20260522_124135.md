...
- `core/agent.py` — 94 lines — last modified: 2026-05-22 12:31:40
...
- `core/nina.py` — 196 lines — last modified: 2026-05-22 12:31:40
...
- `crons/manager.py` — 48 lines — last modified: 2026-05-22 12:31:40
...
- `interfaces/telegram_interface.py` — 353 lines — last modified: 2026-05-22 00:31:10
- `main.py` — 13 lines — last modified: 2026-05-21 21:46:58
...
> Last modified: 2026-05-22 12:31:40 | Lines: 94
...
class AgentLoop:
    ...
    async def _inner(self, goal: str, task: ClassifiedTask, session_history: list) -> str:
        ...
        if cput >= self.config.thermal_critical_cpu or gput >= self.config.thermal_critical_gpu:
            logger.critical(f"agent_loop_aborted_thermal CPU={cput} GPU={gput}",
                            extra={"log":"agent.log"})
        ...
        for step in range(1, max_steps + 1):
            prompt = (
                ...
                f"THINK: reason about the goal.\n"
                f"PLAN: decide next action (tool or respond).\n"
                f"ACT: if tool needed reply with TOOL:<name> INPUT:<input>, else reply FINAL:<answer>"
            )
        ...
        return response  # best effort after max steps
...
> Last modified: 2026-05-22 12:31:40 | Lines: 196

```python
"""NINA v12 — NinaOS orchestrator (Stage 1)."""
import asyncio, fcntl, logging, time
from pathlib import Path
...
from tools import shell, system as systool
...
class NinaOS:
    def __init__(self):
        ...
        self.tools   = {"shell": shell, "web": __import__("tools.web",fromlist=["search"]),
                        "system": systool}
        self.agent   = None
        self.telegram= None
        self.scheduler=None
...
> Last modified: 2026-05-22 12:31:40 | Lines: 48
...
> Last modified: 2026-05-22 00:31:10 | Lines: 353
...
class TelegramInterface:
    ...
    async def _handle_nlp(self, update: Update, text: str):
        ...
        if intent == "general_task":
            ...
            await self._stream_reply(update, text, task)
            return
...
> Last modified: 2026-05-21 21:46:58 | Lines: 13
