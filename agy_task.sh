...
# Usage: ./agy_task.sh <file> "<task description>" ["<do not touch>"] ["<acceptance>"]  
# Example: ./agy_task.sh tools/ninaflash.py "fix the rate limit logic in _handle_quota"
...
if [[ -z "$FILE" || -z "$TASK" ]]; then
  echo "Usage: ./agy_task.sh <file> \"<task>\" [\"<do not touch>\"] [\"<acceptance>\"]"
  exit 1
fi
