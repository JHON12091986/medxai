...
def test_cmd_code_call_graph():
    res = run_nf("code", "call-graph")
    assert res.returncode == 0
    assert '"main.py:acquire_lock":' in res.stdout
    assert (
        '"acquire_lock"' not in res.stdout
    )  # just basic check to ensure calls are gathered correctly
