...
# Usage: ./agy_watchdog.sh   (runs in background; Ctrl+C to stop)
...
while true; do
  ...
  if [[ "$CURRENT_SHA" != "$LAST_SHA" && -n "$CURRENT_SHA" ]]; then
    ...
    # Also check juleslock
    if [[ -f ~/nina/juleslock.txt ]]; then
      while IFS= read -r locked; do
        [[ -z "$locked" ]] && continue
        if echo "$CHANGED" | grep -qF "$locked"; then
          echo "🔒 WATCHDOG ALERT: juleslock file touched in commit $CURRENT_SHA"
          echo "   File   : $locked"
          echo "   Commit : $NEW_COMMIT_MSG"
          ALARM=true
        fi
    ...
    if [[ "$ALARM" == false ]]; then
      echo "✅ $(date +%H:%M:%S) clean commit: $NEW_COMMIT_MSG"
    fi
