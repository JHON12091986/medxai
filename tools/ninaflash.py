...
def run_cmd(cmd, cwd=str(REPO_ROOT), timeout=60) -> Tuple[int, str, str]:
    ...
    try:
        ...
        res = subprocess.run(
            args, shell=False, capture_output=True, text=True, cwd=cwd, timeout=timeout
        )
...
def _safe_run(cmd: str, timeout=60) -> Tuple[int, str, str]:
    ...
    if not is_command_safe(cmd):
        return 1, "", f"❌ SECURITY: Command '{cmd}' blocked."
...
def cmd_edit(args):
    ...
    try:
        ...
        print(
            "Edits must be valid JSON list of dicts: [{'search': 'old', 'replace': 'new'}]"
        )
        sys.exit(1)
...
def cmd_status(args):
    ...
    print(
        f"Backlog: {len(tasks)} total | READY: {len([t for t in tasks if t['status']=='READY'])}"
    )
...
def cmd_maintain_pr(args):
    ...
    code, _, err = run_cmd(
        f"git merge {branch} --no-ff -m 'merge: PR #{pr_id} {task_id}'"
    )
    if code != 0:
...
def _compress_diff(diff_text: str) -> str:
    ...
    return "\n".join(
        [l for l in diff_text.splitlines() if not l.startswith(("---", "+++", "@@"))]
    )
...
def cmd_backlog_triage(args):
    ...
    for task in tasks:
        if task["status"] == "BLOCKED":
            ...
            if blockers and all(
                task_map.get(b, {}).get("status") == "DONE" for b in blockers
            ):
                print(f"✅ Promoting {task['id']} -> READY")
                _save_task_status(task["id"], "READY")
                promoted += 1
    if not promoted:
        print("No tasks eligible for promotion.")
...
def cmd_backlog_add(args):
    ...
    if not args.title or not args.priority:
        print("Error: --title and --priority are required.")
        sys.exit(1)

    tasks = get_backlog_tasks()
    if args.priority == "AG":
        if not args.ag_sub or not args.ag_num:
            print("Error: --ag-sub and --ag-num required for AG priority.")
            sys.exit(1)
    ...
    new_row = (
        f"| {next_id} | {args.title} | `NEEDS_SPEC` | {comp} | — | Added via CLI |\n"
    )
...
def cmd_memory_stash(args):
    ...
    stash.append({"timestamp": datetime.now().isoformat(), "text": text})
...
def cmd_session_checkpoint(args):
    ...
    try:
        ...
        changed_files = (
            [f for f in out_diff.strip().split("\n") if f] if code_diff == 0 else []
        )
...
def cmd_check_code(args):
    ...
    verdict_emoji = (
        "✅ PASS"
        if verdict == "PASS"
        else ("⚠️ WARN" if verdict == "WARN" else "❌ FAIL")
    )
...
def cmd_check_ignore(args):
    ...
    for root, dirs, files in os.walk(REPO_ROOT):
        dirs[:] = [
            d
            for d in dirs
            if not _is_ignored(str((Path(root) / d).relative_to(REPO_ROOT)), patterns)
        ]
        for file in files:
            path = Path(root) / file
...
def cmd_doc_consolidate(args):
    ...
    print(
        f"Target : {log_path.relative_to(REPO_ROOT) if log_path.is_relative_to(REPO_ROOT) else log_path}"
    )
    ...
    if archive_path.exists():
        ...
        archive_path.write_text(
            "# NINA Update Log Archive\n\n" + "\n".join(archive_lines) + "\n",
            encoding="utf-8",
        )

    print(
        f"✅ Moved {len([l for l in archive_lines if l.startswith('## Entry')])} entries to archive."
    )
    print("────────────────────────────────────────")
...
def cmd_hw_gate(args):
    ...
    if getattr(args, "json", False):
        ...
        print(
            f"Details: CPU {data['cpu_temp']}°C | RAM {data['ram_gb']:.1f}GB | Load {data['load']}%"
        )
...
def _get_hw_status() -> Tuple[str, str, dict]:
    ...
    if cpu_temp >= 95 or ram_gb >= (ram_guard + 1.0):
        return (
            "DEFER",
            "Critical thermal/RAM state",
            {"cpu_temp": cpu_temp, "ram_gb": ram_gb, "load": cpu_load},
        )
...
def cmd_register_capability(args):
    ...
    description = (
        args.description or f"Dynamically registered tool from {args.tool_path}"
    )
...
def cmd_stats(args):
    ...
    print(
        f"Backlog: {len(tasks)} tasks | READY: {len([t for t in tasks if t['status']=='READY'])}"
    )
    print(f"Repo .py files: {len(_find_py_files())}")
    cap_status = (
        "✅ UNDER CAP" if fn_count <= 100 else f"🚨 OVER CAP by {fn_count - 100}"
    )
    ...
    ratio = (
        (total_local / (total_local + total_cloud) * 100)
        if (total_local + total_cloud) > 0
        else 0
    )
    ...
    report = {
        ...
        "avg_latency_s": (
            round((total_lat_ms / (total_local + total_cloud + total_cached) / 1000), 2)
            if (total_local + total_cloud + total_cached) > 0
            else 0
        ),
    }
    ...
    try:
        ...
        for node in ast.iter_child_nodes(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                print(
                    f"{sig} {'\"\"\"' + doc.splitlines()[0] + '...\"\"\"' if doc else '...'}"
                )
            elif isinstance(node, ast.ClassDef):
                doc = ast.get_docstring(node)
                print(
                    f"class {node.name}: {'\"\"\"' + doc.splitlines()[0] + '...\"\"\"' if doc else '...'}"
                )
                for item in node.body:
                    if isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef)):
                        ...
                        print(
                            f"{m_sig} {'\"\"\"' + m_doc.splitlines()[0] + '...\"\"\"' if m_doc else '...'}"
                        )
...
def cmd_log_summarize(args):
    ...
    for i, match in enumerate(entries):
        ...
        if i < len(entries) - keep_n:
            if (
                "D-sync Post-session sync" in entry_text
                or "nina_sync.sh v4 automated run" in entry_text
            ):
...
def cmd_file_grep(args):
    ...
    extensions = (
        getattr(args, "ext", ".py,.md,.txt,.log") or ".py,.md,.txt,.log"
    ).split(",")
    patterns = _load_geminiignore()
...
def cmd_file_patch(args):
    ...
    for i, line in enumerate(lines):
        if args.find in line:
            ...
            if path.suffix == ".py":
                run_cmd(f"python3 -m py_compile {path}")
            return
...
def cmd_monitor_tools(sessions_dir, limit):
    ...
    TOOL_TOKEN_COST = {
        "read_file": {
            "avg_in": 800,
            "avg_out": 50,
            "replaceable": "nf file read / nf code symbol",
        },
        "search_files": {
            "avg_in": 1200,
            "avg_out": 80,
            "replaceable": "nf file smart-search",
        },
        "grep_search": {"avg_in": 600, "avg_out": 40, "replaceable": "nf file grep"},
        "list_directory": {
            "avg_in": 200,
            "avg_out": 30,
            "replaceable": "nf code index",
        },
        "run_shell_command": {
            "avg_in": 400,
            "avg_out": 60,
            "replaceable": "nf file/git/log/code commands",
        },
        "write_file": {
            "avg_in": 1000,
            "avg_out": 100,
            "replaceable": "nf file patch / nf file insert",
        },
        "replace": {"avg_in": 300, "avg_out": 50, "replaceable": "nf file patch"},
        "update_topic": {
            "avg_in": 100,
            "avg_out": 20,
            "replaceable": "NOT REPLACEABLE (internal)",
        },
        "save_memory": {
            "avg_in": 150,
            "avg_out": 20,
            "replaceable": "nf memory session-save",
        },
    ...
    session_files = sorted(
        sessions_dir.rglob("*.jsonl"), key=lambda x: x.stat().st_mtime, reverse=True
    )
    parsed = 0
                ...
                for msg in msgs:
                    ...
                    for tc in t_calls:
                        ...
                        if "shell" in name.lower():
                            ...
                            first_word = (
                                cmd_text.strip().split()[0] if cmd_text.strip() else "?"
                            )
    ...
    for tool, count in sorted(tool_counts.items(), key=lambda x: -x[1]):
        meta = TOOL_TOKEN_COST.get(
            tool, {"avg_in": 300, "avg_out": 40, "replaceable": "UNKNOWN"}
        )
    ...
    for tool, count, tokens, cost, replaceable in rows:
        ...
        print(
            f"  {flag} {tool:<24} {count:>6}  {tokens:>9,}  ${cost:>7.4f}  {replaceable}"
        )
    ...
    print(f"  {'-'*68}")
    print(
        f"  TOTAL                          {sum(r[1] for r in rows):>6}  {total_tokens:>9,}  ${total_cost:>7.4f}"
    )
    print()
    print(
        f"  REPLACEABLE with nf commands:  {replaceable_tokens:>9,} tokens  ${replaceable_cost:.4f}"
    )
    ...
    replaceable_tools = [
        r[0] for r in rows if "NOT REPLACEABLE" not in r[4] and r[1] >= 3
    ]
...
def cmd_monitor(args):
            ...
            for line in sf.read_text(encoding="utf-8").splitlines():
                ...
                for msg in msgs:
                    ...
                    for tc in t_calls:
                        ...
                        args_dict = tc.get("args", {})
                        cmd_text = str(args_dict.get("command", ""))
                        if "ninaflash" in cmd_text or cmd_text.strip().startswith(
                            "nf "
                        ):
                            nf_calls += 1
        except Exception:
    ...
    if ninagate_log.exists():
        for line in ninagate_log.read_text().splitlines()[-500:]:
            try:
                ...
                if d.get("cached"):
                    ng_cached += 1
                    ng_tokens_saved += d.get("input_tokens", 0) + d.get(
                        "output_tokens", 0
                    )
    ...
    if tool_calls:
        ...
        for name, count in sorted(tool_calls.items(), key=lambda x: -x[1])[:8]:
            marker = (
                " ← NinaFlash" if ("shell" in name.lower() and nf_calls > 0) else ""
            )
    ...
    if nf_total_calls > 0:
        ...
        print(
            f"  Tokens saved est:   {nf_total_tokens_saved:>6,}  (cloud calls avoided)"
        )
...
def cmd_ops_rotate_logs(args):
    ...
    print(
        f"✅ Rotated and compressed {rotated} log files to {archive_dir.relative_to(REPO_ROOT)}/"
    )
...
def cmd_test(args):
    ...
    print(
        f"Resource Spike: RAM {ram_after - ram_before:+.1f}GB | CPU {cpu_after - cpu_before:+.1f}%"
    )
...
def cmd_gemini_prompt(args):
    try:
        ...
        if args.context_file:
            ...
            try:
                with open(
                    os.path.expanduser(args.context_file), "r", encoding="utf-8"
                ) as f:
...
def main():
    parser = argparse.ArgumentParser(
        description="ninaflash AI Agent Kernel v6.0 — The 100-Function OS."
    )
    ...
    p_monitor.add_argument(
        ...
        action="store_true",
        default=False,
        help="Parse all sessions, not just last 3",
    )
    p_monitor.add_argument(
        "--tools",
        action="store_true",
    ...
    # nf gemini-context
    p_gemini_context = subparsers.add_parser(
        "gemini-context", help="Estimate context tokens for Gemini CLI"
    )

    # nf gemini-prompt
    p_gemini_prompt = subparsers.add_parser(
        "gemini-prompt", help="Generate Gemini CLI prompt file"
    )
    p_gemini_prompt.add_argument("--task", required=True)
    p_gemini_prompt.add_argument("--context-file")

    # nf gemini-status
    p_gemini_status = subparsers.add_parser(
        "gemini-status", help="Show Gemini CLI daily status"
    )

    # nf gemini-run
    p_gemini_run = subparsers.add_parser(
        "gemini-run", help="Run Gemini CLI with dynamic context and model selection"
    )
    ...
    p_file = subparsers.add_parser("file")
    p_fs_f = p_file.add_subparsers(dest="sub")
    p_fr = p_fs_f.add_parser("read")
    p_fr.add_argument("file")
    p_fr.add_argument("--start", type=int)
    p_fr.add_argument("--end", type=int)
    p_fg = p_fs_f.add_parser("grep")
    p_fg.add_argument("pattern")
    p_fg.add_argument("--dir")
    p_fg.add_argument("--ext")
    p_fp = p_fs_f.add_parser("patch")
    p_fp.add_argument("file")
    p_fp.add_argument("--find", required=True)
    p_fp.add_argument("--replace", required=True)
    p_fi = p_fs_f.add_parser("insert")
    p_fi.add_argument("file")
    p_fi.add_argument("--after", required=True)
    p_fi.add_argument("--text", required=True)
    p_fd = p_fs_f.add_parser("diff")
    p_fd.add_argument("file")

    p_git = subparsers.add_parser("git")
    p_gs_g = p_git.add_subparsers(dest="sub")
    p_gl = p_gs_g.add_parser("log")
    p_gl.add_argument("--n", type=int)
    p_gs_g.add_parser("changed")
    p_gs_s = p_gs_g.add_parser("search")
    p_gs_s.add_argument("keyword")
    p_gb = p_gs_g.add_parser("blame")
    p_gb.add_argument("file")
    p_gb.add_argument("--start", type=int)
    p_gb.add_argument("--end", type=int)
    p_gsq = p_gs_g.add_parser("stash-quick")
    p_gsq.add_argument("--label")

    p_hw = subparsers.add_parser("hw")
    p_hs = p_hw.add_subparsers(dest="sub")
    ...
    p_ss = p_session.add_subparsers(dest="sub")
    ...
    p_memory = subparsers.add_parser("memory")
    p_ms = p_memory.add_subparsers(dest="sub")
    p_ms_stash = p_ms.add_parser("stash")
    p_ms_stash.add_argument("text", help="Text to stash in working memory")
    p_mss = p_ms.add_parser("session-save")
    p_mss.add_argument("--summary")
    p_msr = p_ms.add_parser("session-recall")
    p_msr.add_argument("--n", type=int)
    p_ms.add_parser("inject")

    p_check = subparsers.add_parser("check")
    p_cks = p_check.add_subparsers(dest="sub")
    ...
    p_docs = p_doc.add_subparsers(dest="sub")
    ...
    p_cs = p_code.add_subparsers(dest="sub")
    ...
    subparsers.add_parser(
        "bench", help="Run a standardized reasoning task twice (Cloud vs Hybrid)"
    )

    subparsers.add_parser("query", help="List all cmd_ handlers")
    p_query_cap = subparsers.add_parser(
        "query-capability", help="Query task capability"
    )
    p_query_cap.add_argument("task", help="Description of the task to query")

    p_maintain = subparsers.add_parser("maintain")
    p_mts = p_maintain.add_subparsers(dest="sub")
    p_mpr = p_mts.add_parser("pr")
    p_mpr.add_argument("pr_id", help="PR ID to merge")
    p_mpr.add_argument(
        "--task", dest="task_id", required=True, help="Task ID e.g. AG-M-01"
    )
    ...
    p_ps = p_pr.add_subparsers(dest="sub")
    p_ps.add_parser("reconcile")

    p_ops = subparsers.add_parser("ops")
    p_os = p_ops.add_subparsers(dest="sub")
    p_os.add_parser("thermal")
    p_os.add_parser("vram")
    ...
    p_ls = p_log.add_subparsers(dest="sub")
    ...
    p_ba.add_argument(
        "--priority", default=None, choices=["P0", "P1", "P2", "P3", "AG"]
    )
    ...
    p_run.add_argument(
        "extra_args", nargs=argparse.REMAINDER, help="Arguments passed to the tool"
    )
    ...
    try:
        if args.command == "status":
        ...
        elif args.command == "gemini-context":
            cmd_gemini_context(args)
        elif args.command == "gemini-prompt":
            cmd_gemini_prompt(args)
        elif args.command == "gemini-status":
            cmd_gemini_status(args)
        elif args.command == "gemini-run":
            cmd_gemini_run(args)
            ...
            if args.sub == "read":
                ...
                cmd_file_patch(args)
            elif args.sub == "insert":
                cmd_file_insert(args)
            elif args.sub == "diff":
                cmd_file_diff(args)
        elif args.command == "git":
            if args.sub == "log":
                cmd_git_log(args)
            elif args.sub == "changed":
                cmd_git_changed(args)
            elif args.sub == "search":
                cmd_git_search(args)
            elif args.sub == "blame":
                cmd_git_blame(args)
            elif args.sub == "stash-quick":
                cmd_git_stash_quick(args)
        elif args.command == "capability-map":
            cmd_capability_map(args)
        elif args.command == "register":
            cmd_register_capability(args)
        elif args.command == "run":
            cmd_run_capability(args)
        elif args.command == "maintain":
            if args.sub == "pr":
                cmd_maintain_pr(args)
        elif args.command == "session":
            if args.sub == "checkpoint":
                ...
                cmd_session_log(args)
            elif args.sub == "preamble":
                cmd_session_preamble(args)
            elif args.sub == "done":
                cmd_session_done(args)
        elif args.command == "memory":
            if args.sub == "stash":
                cmd_memory_stash(args)
            elif args.sub == "session-save":
                cmd_memory_session_save(args)
            elif args.sub == "session-recall":
                cmd_memory_session_recall(args)
            elif args.sub == "inject":
                cmd_memory_inject(args)
            ...
            if args.sub == "code":
                ...
                cmd_check_doc(args)
            elif args.sub == "ignore":
                cmd_check_ignore(args)
        elif args.command == "doc":
            if args.sub == "check":
                cmd_check_doc(args)
            elif args.sub == "consolidate":
                cmd_doc_consolidate(args)
        elif args.command == "install-hooks":
            cmd_install_hooks(args)
        elif args.command == "code":
            if args.sub == "outline":
                cmd_code_outline(args)
            elif args.sub == "dep-map":
                cmd_code_dep_map(args)
            elif args.sub == "index":
                cmd_code_index(args)
            elif args.sub == "call-graph":
                cmd_code_call_graph(args)
            elif args.sub == "call-stack":
                cmd_code_call_stack(args)
            elif args.sub == "symbol":
                cmd_code_symbol(args)
            elif args.sub == "sigs":
                cmd_code_sigs(args)
            elif args.sub == "doc":
                cmd_code_doc(args)
            ...
            elif args.sub == "dead-code":
                cmd_code_dead_code(args)
        elif args.command == "query":
            cmd_query(args)
        elif args.command == "query-capability":
            cmd_query_capability(args)
        elif args.command == "pr":
            if args.sub == "reconcile":
                cmd_pr_reconcile(args)
        elif args.command == "task":
            if args.sub == "active":
                cmd_task_active(args)
        elif args.command == "log":
            if args.sub == "summarize":
                cmd_log_summarize(args)
            elif args.sub == "tail":
                cmd_log_tail(args)
            elif args.sub == "next-id":
                cmd_log_next_id(args)
            elif args.sub == "find-id":
                cmd_log_find_id(args)
        elif args.command == "backlog":
            if args.sub == "summary":
                cmd_backlog_summary(args)
            elif args.sub == "triage":
                cmd_backlog_triage(args)
            elif args.sub == "dag":
                cmd_backlog_dag(args)
            elif args.sub == "add":
                cmd_backlog_add(args)
            elif args.sub == "archive":
                cmd_backlog_archive(args)
        elif args.command == "ops":
            if args.sub == "thermal":
                cmd_ops_thermal(args)
            elif args.sub == "vram":
                cmd_ops_vram(args)
            elif args.sub == "compress-logs":
                cmd_ops_compress_logs(args)
            elif args.sub == "rotate-logs":
                cmd_ops_rotate_logs(args)
