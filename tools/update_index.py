import json, os, hashlib, ast, sys
...
---
_Generated by NINA Indexer on {DATE}_
"""
...
if __name__ == "__main__":
    ...
    # -------------------------------------------------------------
    # STEP C: Environment Mapping (Dependencies & Symbols)
    # -------------------------------------------------------------
    repo_root = Path(__file__).parent.parent.resolve()
    if str(repo_root) not in sys.path:
        sys.path.append(str(repo_root))
    try:
        from tools.dependency_mapper import generate_dependency_graph
        graph = generate_dependency_graph(repo_root)
        dep_path = repo_root / "data" / "dependency_graph.json"
        dep_path.write_text(json.dumps(graph, indent=2), encoding="utf-8")
        print("✅ Environment Map: Dependency graph updated.")
    except Exception as e:
        print(f"⚠️ Environment Map: Dependency graph failed: {e}")
        
    try:
        from tools.symbol_mapper import generate_symbol_map
        symbol_map = generate_symbol_map(repo_root)
        sym_path = repo_root / "data" / "symbol_map.json"
        sym_path.write_text(json.dumps(symbol_map, indent=2), encoding="utf-8")
        print("✅ Environment Map: Semantic symbol map updated.")
    except Exception as e:
        print(f"⚠️ Environment Map: Semantic symbol map failed: {e}")
